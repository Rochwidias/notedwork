...
export async function POST(req: Request) {
  ...
  if (!title || !body) return Response.json({ error: "Judul & isi wajib diisi" }, { status: 400 });
  ...
  try {
    ...
    const fileId = await upsertNoteDoc(userId, folderId, title.slice(0, 120), body, v.driveFileId);
